...
int calculate_cost(std::string line)
{
	return std::accumulate(line.begin(), line.end(), 0, [&](int n, const char c) {
		return n + lookup_table.at(c);
	});
}
